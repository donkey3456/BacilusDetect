...
CProSingleImage::CProSingleImage(cv::Mat &img)
{
	...
	cv::cvtColor(m_pSrcImg, tmpimg, CV_BGR2HSV, 3); //将原始的BGR图像变换为HSV图像
	...
	m_pClassifiedBin3CImg.setTo(cv::Scalar(0,0,0)); //初始化四个图像数据
...
/********************************************************************************/
	...
	for (int i=0; i < contour.size(); i++)
	{
        ...
        *int iMaxx=Blob.MaxX();
	    ...
	    pt2 = cv::Point2f(aRect.x + aRect.width,aRect.y + aRect.height); //pt2是pt1对角线上的顶点
